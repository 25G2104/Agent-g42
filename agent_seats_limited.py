...
class Group:
    def __init__(self, group_id, size):
        ...
        self.assigned_seats = []
        self.max_stay_time = random.randint(60, 120)  # ← 最大滞在時間（ステップ）
        self.stay_timer = 0
...
class Agent:
    ...
    def update(self):
        ...
        if self.state == 'EATING':
            ...
            self.group.stay_timer += 1 

            if self.group.eating_duration <= 0 or self.group.stay_timer >= self.group.max_stay_time:
                if self.role == 'WAITING':
                    self.path = [(40, 6)]
